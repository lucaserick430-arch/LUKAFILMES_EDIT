...
const USUARIOS_KEY = "usuarios.json";
...
async function carregarUsuarios() {
    ...
    const store = getStore(USUARIOS_STORE);
...
async function salvarUsuarios(usuarios) {
    ...
    if (ambienteLocal) {
        ...
        const inserir = db.prepare(`
            ...
            (id, usuario, senha, status, tipo, validade, criado_em)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        `);
...
app.get('/api/series', async (req, res) => {
    try {
        ...
        const series = Array.isArray(dados.results)
            ? dados.results.map(serie => ({
                id: serie.id,
