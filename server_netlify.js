...
// ==========================================
// TRAILER DA SÉRIE — TMDB
// ==========================================

app.get('/api/serie/:id/videos', async (req, res) => {
    ...
    try {
        ...
        return res.json({
            ...
            trailer: {
                ...
                official: escolhido.official === true,
        ...
        return res.status(500).json({
            ...
            mensagem:
                'Não foi possível buscar o trailer da série.'
...
app.get('/api/filme/:id/videos', async (req, res) => {
    try {
        ...
        const listaTrailers = ordenados
            .filter(video => {
                if (!video || !video.key) return false;

                if (vistos.has(video.key)) {
                    return false;
                }

                vistos.add(video.key);
                return true;
            })
            .slice(0, 100);

        const trailersFormatados = listaTrailers.map(video => ({
            id: video.id,
            nome: video.name,
            key: video.key,
            chave: video.key,
            ...
            oficial: video.official === true,
            idioma: video.iso_639_1 || null,
            url: 'https://www.youtube.com/watch?v=' + video.key,
            embed: 'https://www.youtube.com/embed/' + video.key
        }));

        if (!trailersFormatados.length) {
            return res.json({
                sucesso: true,
                encontrado: false,
                trailer: null,
        ...
        return res.json({
            sucesso: true,
            encontrado: true,
            trailer: trailersFormatados[0],
            trailers: trailersFormatados,
            total: trailersFormatados.length
        });

    } catch (erro) {
        console.error('[ERRO TRAILERS TMDB]', erro);
        ...
        return res.status(500).json({
            ...
            trailers: [],
            total: 0,
            mensagem: 'Não foi possível buscar os trailers.'
        });
    }
});
// ==========================================
...
app.use(
    (req, res, next) => {
        ...
        if (
            ...
            req.path === '/api/catalogo' || req.path.startsWith('/api/serie/') || req.path === '/paginas/filmes.html' || req.path === '/paginas/filme.html' || req.path === '/paginas/filme' || req.path === '/paginas/series.html'
...
// TESTE LOCAL TEMPORARIO
if (require.main === module) {
  app.listen(PORT, "0.0.0.0", () => {
    console.log("========================================");
    console.log("✅ SERVIDOR LUKAFILMES COM TRAILERS");
