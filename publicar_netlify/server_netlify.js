...
const ambienteLocal = !process.env.NETLIFY;
...
// ==========================================================
// ADMIN DASHBOARD — LISTAGEM E PRESENÇA
// ==========================================================
const presencaAdmin = new Map();

app.use((req, res, next) => {
    try {
        if (req.session && req.session.usuario) {
            const id = Number(req.session.usuario.id);
            if (Number.isInteger(id) && id > 0) {
                presencaAdmin.set(id, Date.now());
            }
        }
    } catch (_) {}
    next();
...
app.get("/api/admin/usuarios", async (req, res) => {
    try {
        ...
        const lista = usuarios.map(u => {
            ...
            const ultima = presencaAdmin.get(id) || 0;
            const online = (agora - ultima) <= 5 * 60 * 1000;
            ...
            return {
                ...
                limite_conexoes: u.limite_conexoes || 0,
                conexoes_utilizadas: u.conexoes_utilizadas || 0,
                online,
                ultima_atividade: ultima || null
            };
        });
        ...
        console.error("[ERRO LISTAR USUARIOS ADMIN]", erro);
...
app.post("/api/admin/presenca", async (req, res) => {
    try {
        if (!req.session.usuario) {
            return res.status(403).json({
                sucesso: false
...
app.get("/revendedor.html", (req, res) => {
    res.sendFile(
        path.join(process.cwd(), "public", "revendedor.html")
    );
});
