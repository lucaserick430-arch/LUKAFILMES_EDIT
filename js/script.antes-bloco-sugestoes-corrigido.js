...
async function pesquisarSugestoes(termo) {
    ...
    const pesquisaAtual = ++numeroPesquisaSugestao;
    const campo = document.getElementById('campoBusca');
    if (!campo) return;
    ...
    timerSugestoes = setTimeout(async () => {
        ...
        if (pesquisaAtual !== numeroPesquisaSugestao) {
            return;
        }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    campoSugestao.addEventListener("input", function () {
        ...
        // Se a barra ficou vazia,
        // apaga imediatamente as sugestões.
        if (!textoAtual) {
